...
PROJECT_NAME="protocol.sh"

PROJECT_ENTRY="protocol.sh"

PROJECT="https://github.com/khulnasoft/$PROJECT_NAME"
...
CA_LETSENCRYPT_V2="https://protocol-v02.api.letsencrypt.org/directory"
CA_LETSENCRYPT_V2_TEST="https://protocol-staging-v02.api.letsencrypt.org/directory"

CA_BUYPASS="https://api.buypass.com/protocol/directory"
CA_BUYPASS_TEST="https://api.test4.buypass.no/protocol/directory"

CA_ZEROSSL="https://protocol.zerossl.com/v2/DV90"
_ZERO_EAB_ENDPOINT="https://api.zerossl.com/protocol/eab-credentials-email"

CA_SSLCOM_RSA="https://protocol.ssl.com/sslcom-dv-rsa"
CA_SSLCOM_ECC="https://protocol.ssl.com/sslcom-dv-ecc"

CA_GOOGLE="https://dv.protocol-v02.api.pki.goog/directory"
CA_GOOGLE_TEST="https://dv.protocol-v02.test-api.pki.goog/directory"
...
NGINX="nginx:"
NGINX_START="#PROTOCOL_NGINX_START"
NGINX_END="#PROTOCOL_NGINX_END"
...
B64CONF_START="__PROTOCOL_BASE64__START_"
B64CONF_END="__PROTOCOL_BASE64__END_"
...
_DEBUG_WIKI="https://github.com/khulnasoft/protocol.sh/wiki/How-to-debug-protocol.sh"

_PREPARE_LINK="https://github.com/khulnasoft/protocol.sh/wiki/Install-preparations"

_STATELESS_WIKI="https://github.com/khulnasoft/protocol.sh/wiki/Stateless-Mode"

_DNS_ALIAS_WIKI="https://github.com/khulnasoft/protocol.sh/wiki/DNS-alias-mode"

_DNS_MANUAL_WIKI="https://github.com/khulnasoft/protocol.sh/wiki/dns-manual-mode"

_DNS_API_WIKI="https://github.com/khulnasoft/protocol.sh/wiki/dnsapi"

_NOTIFY_WIKI="https://github.com/khulnasoft/protocol.sh/wiki/notify"

_SUDO_WIKI="https://github.com/khulnasoft/protocol.sh/wiki/sudo"

_REVOKE_WIKI="https://github.com/khulnasoft/protocol.sh/wiki/revokecert"

_ZEROSSL_WIKI="https://github.com/khulnasoft/protocol.sh/wiki/ZeroSSL.com-CA"

_SSLCOM_WIKI="https://github.com/khulnasoft/protocol.sh/wiki/SSL.com-CA"

_SERVER_WIKI="https://github.com/khulnasoft/protocol.sh/wiki/Server"

_PREFERRED_CHAIN_WIKI="https://github.com/khulnasoft/protocol.sh/wiki/Preferred-Chain"

_VALIDITY_WIKI="https://github.com/khulnasoft/protocol.sh/wiki/Validity"

_DNSCHECK_WIKI="https://github.com/khulnasoft/protocol.sh/wiki/dnscheck"
...
__green() {
  if [ "${__INTERACTIVE}${PROTOCOL_NO_COLOR:-0}" = "10" -o "${PROTOCOL_FORCE_COLOR}" = "1" ]; then
...
_dlg_versions() {
  ...
  echo "openssl:$PROTOCOL_OPENSSL_BIN"
  if _exists "${PROTOCOL_OPENSSL_BIN:-openssl}"; then
    ${PROTOCOL_OPENSSL_BIN:-openssl} version 2>&1
  else
    echo "$PROTOCOL_OPENSSL_BIN doesn't exist."
  fi
...
_base64() {
  ...
  if [ "$1" ]; then
    ...
    ${PROTOCOL_OPENSSL_BIN:-openssl} base64 -e
  else
    _debug3 "base64 single line."
    ${PROTOCOL_OPENSSL_BIN:-openssl} base64 -e | tr -d '\r\n'
  fi
}
...
_dbase64() {
  if [ "$1" ]; then
    ${PROTOCOL_OPENSSL_BIN:-openssl} base64 -d
  else
    ${PROTOCOL_OPENSSL_BIN:-openssl} base64 -d -A
  fi
}
...
_checkcert() {
  _cf="$1"
  if [ "$DEBUG" ]; then
    ${PROTOCOL_OPENSSL_BIN:-openssl} x509 -noout -text -in "$_cf"
  else
    ${PROTOCOL_OPENSSL_BIN:-openssl} x509 -noout -text -in "$_cf" >/dev/null 2>&1
  fi
}
...
_digest() {
  ...
  if [ "$alg" = "sha256" ] || [ "$alg" = "sha1" ] || [ "$alg" = "md5" ]; then
    if [ "$outputhex" ]; then
      ${PROTOCOL_OPENSSL_BIN:-openssl} dgst -"$alg" -hex | cut -d = -f 2 | tr -d ' '
    else
      ${PROTOCOL_OPENSSL_BIN:-openssl} dgst -"$alg" -binary | _base64
    fi
  else
...
_hmac() {
  ...
  if [ "$alg" = "sha256" ] || [ "$alg" = "sha1" ]; then
    if [ "$outputhex" ]; then
      (${PROTOCOL_OPENSSL_BIN:-openssl} dgst -"$alg" -mac HMAC -macopt "hexkey:$secret_hex" 2>/dev/null || ${PROTOCOL_OPENSSL_BIN:-openssl} dgst -"$alg" -hmac "$(printf "%s" "$secret_hex" | _h2b)") | cut -d = -f 2 | tr -d ' '
    else
      ${PROTOCOL_OPENSSL_BIN:-openssl} dgst -"$alg" -mac HMAC -macopt "hexkey:$secret_hex" -binary 2>/dev/null || ${PROTOCOL_OPENSSL_BIN:-openssl} dgst -"$alg" -hmac "$(printf "%s" "$secret_hex" | _h2b)" -binary
    fi
  else
    _err "$alg is not supported yet"
    return 1
...
_sign() {
  ...
  _sign_openssl="${PROTOCOL_OPENSSL_BIN:-openssl} dgst -sign $keyfile "

  if _isRSA "$keyfile" >/dev/null 2>&1; then
    $_sign_openssl -$alg | _base64
  elif _isEcc "$keyfile" >/dev/null 2>&1; then
    if ! _signedECText="$($_sign_openssl -sha$__ECC_KEY_LEN | ${PROTOCOL_OPENSSL_BIN:-openssl} asn1parse -inform DER)"; then
...
_createkey() {
  ...
  if _isEccKey "$length"; then
    ...
    if _opkey="$(${PROTOCOL_OPENSSL_BIN:-openssl} ecparam -name "$eccname" -noout -genkey 2>/dev/null)"; then
      echo "$_opkey" >"$f"
    else
    ...
    __traditional=""
    if _contains "$(${PROTOCOL_OPENSSL_BIN:-openssl} help genrsa 2>&1)" "-traditional"; then
      __traditional="-traditional"
    fi
    if _opkey="$(${PROTOCOL_OPENSSL_BIN:-openssl} genrsa $__traditional "$length" 2>/dev/null)"; then
      echo "$_opkey" >"$f"
    else
...
#_createcsr  cn  san_list  keyfile csrfile conf protocolValidationv1
_createcsr() {
  _debug _createcsr
  domain="$1"
  domainlist="$2"
  ...
  if [ "$protocolValidationv1" ]; then
    printf "\n1.3.6.1.5.5.7.1.31=critical,DER:04:20:${protocolValidationv1}" >>"${csrconf}"
  fi
  ...
  if _contains "$(uname -a)" "MINGW"; then
    if _isIP "$_csr_cn"; then
      ${PROTOCOL_OPENSSL_BIN:-openssl} req -new -sha256 -key "$csrkey" -subj "//O=$PROJECT_NAME" -config "$csrconf" -out "$csr"
    else
      ${PROTOCOL_OPENSSL_BIN:-openssl} req -new -sha256 -key "$csrkey" -subj "//CN=$_csr_cn" -config "$csrconf" -out "$csr"
    fi
  else
    if _isIP "$_csr_cn"; then
      ${PROTOCOL_OPENSSL_BIN:-openssl} req -new -sha256 -key "$csrkey" -subj "/O=$PROJECT_NAME" -config "$csrconf" -out "$csr"
...
_signcsr() {
  ...
  _msg="$(${PROTOCOL_OPENSSL_BIN:-openssl} x509 -req -days 365 -in "$csr" -signkey "$key" -extensions v3_req -extfile "$conf" -out "$cert" 2>&1)"
  _ret="$?"
  _debug "$_msg"
  return $_ret
}
...
_readSubjectFromCSR() {
  ...
  ${PROTOCOL_OPENSSL_BIN:-openssl} req -noout -in "$_csrfile" -subject | tr ',' "\n" | _egrep_o "CN *=.*" | cut -d = -f 2 | cut -d / -f 1 | tr -d ' \n'
}
...
_readSubjectAltNamesFromCSR() {
  ...
  _dnsAltnames="$(${PROTOCOL_OPENSSL_BIN:-openssl} req -noout -text -in "$_csrfile" | grep "^ *DNS:.*" | tr -d ' \n')"
...
_readKeyLengthFromCSR() {
  ...
  _outcsr="$(${PROTOCOL_OPENSSL_BIN:-openssl} req -noout -text -in "$_csrfile")"
...
_toPkcs() {
  ...
  if [ "$pfxCaname" ]; then
    ${PROTOCOL_OPENSSL_BIN:-openssl} pkcs12 -export -out "$_cpfx" -inkey "$_ckey" -in "$_ccert" -certfile "$_cca" -password "pass:$pfxPassword" -name "$pfxName" -caname "$pfxCaname"
  elif [ "$pfxName" ]; then
    ${PROTOCOL_OPENSSL_BIN:-openssl} pkcs12 -export -out "$_cpfx" -inkey "$_ckey" -in "$_ccert" -certfile "$_cca" -password "pass:$pfxPassword" -name "$pfxName"
  elif [ "$pfxPassword" ]; then
    ${PROTOCOL_OPENSSL_BIN:-openssl} pkcs12 -export -out "$_cpfx" -inkey "$_ckey" -in "$_ccert" -certfile "$_cca" -password "pass:$pfxPassword"
  else
    ${PROTOCOL_OPENSSL_BIN:-openssl} pkcs12 -export -out "$_cpfx" -inkey "$_ckey" -in "$_ccert" -certfile "$_cca"
  fi
  if [ "$?" == "0" ]; then
...
toPkcs8() {
  ...
  ${PROTOCOL_OPENSSL_BIN:-openssl} pkcs8 -topk8 -inform PEM -outform PEM -nocrypt -in "$CERT_KEY_PATH" -out "$CERT_PKCS8_PATH"
...
createDomainKey() {
  ...
  if [ ! -f "$CERT_KEY_PATH" ] || [ ! -s "$CERT_KEY_PATH" ] || ([ "$FORCE" ] && ! [ "$_PROTOCOL_IS_RENEW" ]) || [ "$Le_ForceNewDomainKey" = "1" ]; then
...
createCSR() {
  ...
  if [ -f "$CSR_PATH" ] && [ "$_PROTOCOL_IS_RENEW" ] && [ -z "$FORCE" ]; then
...
_isRSA() {
  keyfile=$1
  if grep "BEGIN RSA PRIVATE KEY" "$keyfile" >/dev/null 2>&1 || ${PROTOCOL_OPENSSL_BIN:-openssl} rsa -in "$keyfile" -noout -text 2>&1 | grep "^publicExponent:" 2>&1 >/dev/null; then
    return 0
  fi
  return 1
}
...
_isEcc() {
  keyfile=$1
  if grep "BEGIN EC PRIVATE KEY" "$keyfile" >/dev/null 2>&1 || ${PROTOCOL_OPENSSL_BIN:-openssl} ec -in "$keyfile" -noout -text 2>/dev/null | grep "^NIST CURVE:" 2>&1 >/dev/null; then
    return 0
  fi
  return 1
}
...
_calcjwk() {
  ...
  if _isRSA "$keyfile"; then
    _debug "RSA key"
    pub_exp=$(${PROTOCOL_OPENSSL_BIN:-openssl} rsa -in "$keyfile" -noout -text | grep "^publicExponent:" | cut -d '(' -f 2 | cut -d 'x' -f 2 | cut -d ')' -f 1)
    if [ "${#pub_exp}" = "5" ]; then
      pub_exp=0$pub_exp
    fi
    ...
    modulus=$(${PROTOCOL_OPENSSL_BIN:-openssl} rsa -in "$keyfile" -modulus -noout | cut -d '=' -f 2)
    ...
    _debug "EC key"
    crv="$(${PROTOCOL_OPENSSL_BIN:-openssl} ec -in "$keyfile" -noout -text 2>/dev/null | grep "^NIST CURVE:" | cut -d ":" -f 2 | tr -d " \r\n")"
    _debug3 crv "$crv"
    ...
    if [ -z "$crv" ]; then
      ...
      crv_oid="$(${PROTOCOL_OPENSSL_BIN:-openssl} ec -in "$keyfile" -noout -text 2>/dev/null | grep "^ASN1 OID:" | cut -d ":" -f 2 | tr -d " \r\n")"
    ...
    pubi="$(${PROTOCOL_OPENSSL_BIN:-openssl} ec -in "$keyfile" -noout -text 2>/dev/null | grep -n pub: | cut -d : -f 1)"
    pubi=$(_math "$pubi" + 1)
    _debug3 pubi "$pubi"

    pubj="$(${PROTOCOL_OPENSSL_BIN:-openssl} ec -in "$keyfile" -noout -text 2>/dev/null | grep -n "ASN1 OID:" | cut -d : -f 1)"
    pubj=$(_math "$pubj" - 1)
    _debug3 pubj "$pubj"

    pubtext="$(${PROTOCOL_OPENSSL_BIN:-openssl} ec -in "$keyfile" -noout -text 2>/dev/null | sed -n "$pubi,${pubj}p" | tr -d " \n\r")"
...
_inithttp() {
  ...
  if [ "$__HTTP_INITIALIZED" ]; then
    if [ "$_PROTOCOL_CURL$_PROTOCOL_WGET" ]; then
      _debug2 "Http already initialized."
      return 0
    fi
  fi

  if [ -z "$_PROTOCOL_CURL" ] && _exists "curl"; then
    _PROTOCOL_CURL="curl --silent --dump-header $HTTP_HEADER "
    if [ -z "$PROTOCOL_HTTP_NO_REDIRECTS" ]; then
      _PROTOCOL_CURL="$_PROTOCOL_CURL -L "
    fi
    if [ "$DEBUG" ] && [ "$DEBUG" -ge 2 ]; then
      _CURL_DUMP="$(_mktemp)"
      _PROTOCOL_CURL="$_PROTOCOL_CURL --trace-ascii $_CURL_DUMP "
    fi

    if [ "$CA_PATH" ]; then
      _PROTOCOL_CURL="$_PROTOCOL_CURL --capath $CA_PATH "
    elif [ "$CA_BUNDLE" ]; then
      _PROTOCOL_CURL="$_PROTOCOL_CURL --cacert $CA_BUNDLE "
    fi

    if _contains "$(curl --help 2>&1)" "--globoff" || _contains "$(curl --help curl 2>&1)" "--globoff"; then
      _PROTOCOL_CURL="$_PROTOCOL_CURL -g "
    fi
    ...
    #  _PROTOCOL_CURL="$_PROTOCOL_CURL --fail-with-body "
    #fi
  fi

  if [ -z "$_PROTOCOL_WGET" ] && _exists "wget"; then
    _PROTOCOL_WGET="wget -q"
    if [ "$PROTOCOL_HTTP_NO_REDIRECTS" ]; then
      _PROTOCOL_WGET="$_PROTOCOL_WGET --max-redirect 0 "
    fi
    if [ "$DEBUG" ] && [ "$DEBUG" -ge "2" ]; then
      if [ "$_PROTOCOL_WGET" ] && _contains "$($_PROTOCOL_WGET --help 2>&1)" "--debug"; then
        _PROTOCOL_WGET="$_PROTOCOL_WGET -d "
      fi
    fi
    if [ "$CA_PATH" ]; then
      _PROTOCOL_WGET="$_PROTOCOL_WGET --ca-directory=$CA_PATH "
    elif [ "$CA_BUNDLE" ]; then
      _PROTOCOL_WGET="$_PROTOCOL_WGET --ca-certificate=$CA_BUNDLE "
    fi

    #from wget 1.14: do not skip body on 404 error
    if _contains "$(wget --help 2>&1)" "--content-on-error"; then
      _PROTOCOL_WGET="$_PROTOCOL_WGET --content-on-error "
    fi
  fi
...
_post() {
  ...
  if [ "$_PROTOCOL_CURL" ] && [ "${PROTOCOL_USE_WGET:-0}" = "0" ]; then
    _CURL="$_PROTOCOL_CURL"
  ...
  elif [ "$_PROTOCOL_WGET" ]; then
    _WGET="$_PROTOCOL_WGET"
    if [ "$HTTPS_INSECURE" ]; then
...
_send_signed_request() {
  ...
  while [ "${_request_retry_times}" -lt "$MAX_REQUEST_RETRY_TIMES" ]; do
    ...
    if [ -z "$_CACHED_NONCE" ]; then
      _headers=""
      if [ "$PROTOCOL_NEW_NONCE" ]; then
        _debug2 "Get nonce with HEAD. PROTOCOL_NEW_NONCE" "$PROTOCOL_NEW_NONCE"
        nonceurl="$PROTOCOL_NEW_NONCE"
      ...
      if [ -z "$_CACHED_NONCE" ]; then
        _debug2 "Get nonce with GET. PROTOCOL_DIRECTORY" "$PROTOCOL_DIRECTORY"
        nonceurl="$PROTOCOL_DIRECTORY"
        _headers="$(_get "$nonceurl" "onlyheader")"
        _debug2 _headers "$_headers"
        _CACHED_NONCE="$(echo "$_headers" | grep -i "Replay-Nonce:" | _head_n 1 | tr -d "\r\n " | cut -d ':' -f 2)"
      fi
      if [ -z "$_CACHED_NONCE" ] && [ "$PROTOCOL_NEW_NONCE" ]; then
        _debug2 "Get nonce with GET. PROTOCOL_NEW_NONCE" "$PROTOCOL_NEW_NONCE"
        nonceurl="$PROTOCOL_NEW_NONCE"
        _headers="$(_get "$nonceurl" "onlyheader")"
        _debug2 _headers "$_headers"
        _CACHED_NONCE="$(echo "$_headers" | grep -i "Replay-Nonce:" | _head_n 1 | tr -d "\r\n " | cut -d ':' -f 2)"
    ...
    if [ "$url" = "$PROTOCOL_NEW_ACCOUNT" ]; then
      protected="$JWK_HEADERPLACE_PART1$nonce\", \"url\": \"${url}$JWK_HEADERPLACE_PART2, \"jwk\": $jwk"'}'
    elif [ "$url" = "$PROTOCOL_REVOKE_CERT" ] && [ "$keyfile" != "$ACCOUNT_KEY_PATH" ]; then
      protected="$JWK_HEADERPLACE_PART1$nonce\", \"url\": \"${url}$JWK_HEADERPLACE_PART2, \"jwk\": $jwk"'}'
    else
...
# _starttlsserver  san_a  san_b port content _ncaddr protocolValidationv1
_starttlsserver() {
  ...
  _debug san_b "$san_b"
  _debug port "$port"
  _debug protocolValidationv1 "$protocolValidationv1"
  ...
  if ! _createcsr "tls.protocol.sh" "$alt" "$TLS_KEY" "$TLS_CSR" "$TLS_CONF" "$protocolValidationv1"; then
  ...
  __S_OPENSSL="${PROTOCOL_OPENSSL_BIN:-openssl} s_server -www -cert $TLS_CERT  -key $TLS_KEY "
  if [ "$opaddr" ]; then
  ...
  if [ "$protocolValidationv1" ]; then
    __S_OPENSSL="$__S_OPENSSL -alpn protocol-tls/1"
  fi
...
_clearAPI() {
  PROTOCOL_NEW_ACCOUNT=""
  PROTOCOL_KEY_CHANGE=""
  PROTOCOL_NEW_AUTHZ=""
  PROTOCOL_NEW_ORDER=""
  PROTOCOL_REVOKE_CERT=""
  PROTOCOL_NEW_NONCE=""
  PROTOCOL_AGREEMENT=""
}
...
#server
_initAPI() {
  _api_server="${1:-$PROTOCOL_DIRECTORY}"
  _debug "_init API for server: $_api_server"

  MAX_API_RETRY_TIMES=10
  _sleep_retry_sec=10
  _request_retry_times=0
  while [ -z "$PROTOCOL_NEW_ACCOUNT" ] && [ "${_request_retry_times}" -lt "$MAX_API_RETRY_TIMES" ]; do
    _request_retry_times=$(_math "$_request_retry_times" + 1)
    ...
    PROTOCOL_KEY_CHANGE=$(echo "$response" | _egrep_o 'keyChange" *: *"[^"]*"' | cut -d '"' -f 3)
    export PROTOCOL_KEY_CHANGE

    PROTOCOL_NEW_AUTHZ=$(echo "$response" | _egrep_o 'newAuthz" *: *"[^"]*"' | cut -d '"' -f 3)
    export PROTOCOL_NEW_AUTHZ

    PROTOCOL_NEW_ORDER=$(echo "$response" | _egrep_o 'newOrder" *: *"[^"]*"' | cut -d '"' -f 3)
    export PROTOCOL_NEW_ORDER

    PROTOCOL_NEW_ACCOUNT=$(echo "$response" | _egrep_o 'newAccount" *: *"[^"]*"' | cut -d '"' -f 3)
    export PROTOCOL_NEW_ACCOUNT

    PROTOCOL_REVOKE_CERT=$(echo "$response" | _egrep_o 'revokeCert" *: *"[^"]*"' | cut -d '"' -f 3)
    export PROTOCOL_REVOKE_CERT

    PROTOCOL_NEW_NONCE=$(echo "$response" | _egrep_o 'newNonce" *: *"[^"]*"' | cut -d '"' -f 3)
    export PROTOCOL_NEW_NONCE

    PROTOCOL_AGREEMENT=$(echo "$response" | _egrep_o 'termsOfService" *: *"[^"]*"' | cut -d '"' -f 3)
    export PROTOCOL_AGREEMENT

    _debug "PROTOCOL_KEY_CHANGE" "$PROTOCOL_KEY_CHANGE"
    _debug "PROTOCOL_NEW_AUTHZ" "$PROTOCOL_NEW_AUTHZ"
    _debug "PROTOCOL_NEW_ORDER" "$PROTOCOL_NEW_ORDER"
    _debug "PROTOCOL_NEW_ACCOUNT" "$PROTOCOL_NEW_ACCOUNT"
    _debug "PROTOCOL_REVOKE_CERT" "$PROTOCOL_REVOKE_CERT"
    _debug "PROTOCOL_AGREEMENT" "$PROTOCOL_AGREEMENT"
    _debug "PROTOCOL_NEW_NONCE" "$PROTOCOL_NEW_NONCE"
    if [ "$PROTOCOL_NEW_ACCOUNT" ] && [ "$PROTOCOL_NEW_ORDER" ]; then
      return 0
    fi
    _info "Sleeping for $_sleep_retry_sec seconds and retrying."
    _sleep "$_sleep_retry_sec"
...
_initpath() {
  ...
  if [ -z "$PROTOCOL_DIRECTORY" ]; then
    if [ "$STAGE" ]; then
      PROTOCOL_DIRECTORY="$DEFAULT_STAGING_CA"
      _info "Using PROTOCOL_DIRECTORY: $PROTOCOL_DIRECTORY"
    else
      default_protocol_server=$(_readaccountconf "DEFAULT_PROTOCOL_SERVER")
      _debug default_protocol_server "$default_protocol_server"
      if [ "$default_protocol_server" ]; then
        PROTOCOL_DIRECTORY="$default_protocol_server"
      else
        PROTOCOL_DIRECTORY="$DEFAULT_CA"
      fi
    fi
  fi

  _debug PROTOCOL_DIRECTORY "$PROTOCOL_DIRECTORY"
  _PROTOCOL_SERVER_HOST="$(echo "$PROTOCOL_DIRECTORY" | cut -d : -f 2 | tr -s / | cut -d / -f 2)"
  _debug2 "_PROTOCOL_SERVER_HOST" "$_PROTOCOL_SERVER_HOST"

  _PROTOCOL_SERVER_PATH="$(echo "$PROTOCOL_DIRECTORY" | cut -d : -f 2- | tr -s / | cut -d / -f 3-)"
  _debug2 "_PROTOCOL_SERVER_PATH" "$_PROTOCOL_SERVER_PATH"

  CA_DIR="$CA_HOME/$_PROTOCOL_SERVER_HOST/$_PROTOCOL_SERVER_PATH"
  ...
  _OLD_CADIR="$CA_HOME/$_PROTOCOL_SERVER_HOST"
  ...
  if [ -z "$PROTOCOL_OPENSSL_BIN" ] || [ ! -f "$PROTOCOL_OPENSSL_BIN" ] || [ ! -x "$PROTOCOL_OPENSSL_BIN" ]; then
    PROTOCOL_OPENSSL_BIN="$DEFAULT_OPENSSL_BIN"
  fi
...
_setApache() {
  ...
  if [ "$apacheVer" ] && [ "$apacheMajor$apacheMinor" -ge "24" ]; then
    echo "
Alias /.well-known/protocol-challenge  $PROTOCOL_DIR
...
_setNginx() {
  ...
  echo "$NGINX_START
location ~ \"^/\.well-known/protocol-challenge/([-_a-zA-Z0-9]+)\$\" {
...
_clearupwebbroot() {
  ...
  if [ "$2" = '1' ]; then
    ...
    _rmpath="$__webroot/.well-known/protocol-challenge"
  elif [ "$2" = '3' ]; then
    _rmpath="$__webroot/.well-known/protocol-challenge/$3"
  else
...
_on_issue_err() {
  ...
  if [ "$_PROTOCOL_IS_RENEW" = "1" ] && _hasfield "$Le_Webroot" "$W_DNS"; then
...
_on_issue_success() {
  ...
  #run renew hook
  if [ "$_PROTOCOL_IS_RENEW" ] && [ "$_chk_renew_hook" ]; then
...
_regAccount() {
  ...
  if [ "$PROTOCOL_DIRECTORY" = "$CA_ZEROSSL" ]; then
  ...
  if [ "$_eab_id" ] && [ "$_eab_hmac_key" ]; then
    eab_protected="{\"alg\":\"HS256\",\"kid\":\"$_eab_id\",\"url\":\"${PROTOCOL_NEW_ACCOUNT}\"}"
  ...
  _info "Registering account: $PROTOCOL_DIRECTORY"

  if ! _send_signed_request "${PROTOCOL_NEW_ACCOUNT}" "$regjson"; then
...
__get_domain_new_authz() {
  ...
  while [ "$_authz_i" -lt "$_Max_new_authz_retry_times" ]; do
    ...
    if ! _send_signed_request "${PROTOCOL_NEW_AUTHZ}" "{\"resource\": \"new-authz\", \"identifier\": {\"type\": \"dns\", \"value\": \"$(_idn "$_gdnd")\"}}"; then
...
_get_chain_issuers() {
  _cfile="$1"
  if _contains "$(${PROTOCOL_OPENSSL_BIN:-openssl} help crl2pkcs7 2>&1)" "Usage: crl2pkcs7" || _contains "$(${PROTOCOL_OPENSSL_BIN:-openssl} crl2pkcs7 -help 2>&1)" "Usage: crl2pkcs7" || _contains "$(${PROTOCOL_OPENSSL_BIN:-openssl} crl2pkcs7 help 2>&1)" "unknown option help"; then
    ${PROTOCOL_OPENSSL_BIN:-openssl} crl2pkcs7 -nocrl -certfile $_cfile | ${PROTOCOL_OPENSSL_BIN:-openssl} pkcs7 -print_certs -text -noout | grep -i 'Issuer:' | _egrep_o "CN *=[^,]*" | cut -d = -f 2
  else
    _cindex=1
    for _startn in $(grep -n -- "$BEGIN_CERT" "$_cfile" | cut -d : -f 1); do
      ...
      sed -n "$_startn,${_endn}p" "$_cfile" | ${PROTOCOL_OPENSSL_BIN:-openssl} x509 -text -noout | grep 'Issuer:' | _egrep_o "CN *=[^,]*" | cut -d = -f 2 | sed "s/ *\(.*\)/\1/"
...
_get_chain_subjects() {
  _cfile="$1"
  if _contains "$(${PROTOCOL_OPENSSL_BIN:-openssl} help crl2pkcs7 2>&1)" "Usage: crl2pkcs7" || _contains "$(${PROTOCOL_OPENSSL_BIN:-openssl} crl2pkcs7 -help 2>&1)" "Usage: crl2pkcs7" || _contains "$(${PROTOCOL_OPENSSL_BIN:-openssl} crl2pkcs7 help 2>&1)" "unknown option help"; then
    ${PROTOCOL_OPENSSL_BIN:-openssl} crl2pkcs7 -nocrl -certfile $_cfile | ${PROTOCOL_OPENSSL_BIN:-openssl} pkcs7 -print_certs -text -noout | grep -i 'Subject:' | _egrep_o "CN *=[^,]*" | cut -d = -f 2
  else
    _cindex=1
    for _startn in $(grep -n -- "$BEGIN_CERT" "$_cfile" | cut -d : -f 1); do
      _endn="$(grep -n -- "$END_CERT" "$_cfile" | cut -d : -f 1 | _head_n $_cindex | _tail_n 1)"
      ...
      sed -n "$_startn,${_endn}p" "$_cfile" | ${PROTOCOL_OPENSSL_BIN:-openssl} x509 -text -noout | grep -i 'Subject:' | _egrep_o "CN *=[^,]*" | cut -d = -f 2 | sed "s/ *\(.*\)/\1/"
      _cindex=$(_math $_cindex + 1)
    done
  fi
}
...
issue() {
  ...
  _debug "Using PROTOCOL_DIRECTORY: $PROTOCOL_DIRECTORY"
  if ! _initAPI; then
    return 1
  fi
  ...
  Le_API="$PROTOCOL_DIRECTORY"
  _savedomainconf "Le_API" "$Le_API"

  _info "Using CA: $PROTOCOL_DIRECTORY"
  ...
  if [ -z "$vlist" ]; then
    ...
    if ! _send_signed_request "$PROTOCOL_NEW_ORDER" "$_newOrderObj}"; then
      ...
      if [ "$vtype" = "$VTYPE_DNS" ]; then
        ...
        if [ "$_d_alias" ]; then
          if _startswith "$_d_alias" "$DNS_ALIAS_PREFIX"; then
            ...
            txtdomain="_protocol-challenge.$_d_alias"
          fi
          dns_entry="${_dns_root_d}${dvsep}_protocol-challenge.$_dns_root_d$dvsep$txtdomain$dvsep$_currentRoot"
        else
          txtdomain="_protocol-challenge.$_dns_root_d"
          dns_entry="${_dns_root_d}${dvsep}_protocol-challenge.$_dns_root_d$dvsep$dvsep$_currentRoot"
        fi
        ...
        if [ "$d_api" ]; then
          ...
          _info "$(__red "Please make sure to prepend '_protocol-challenge.' to your domain")"
...
"
    ...
    if [ "$vtype" = "$VTYPE_HTTP" ]; then
      if [ "$_currentRoot" = "$NO_VALUE" ]; then
        ...
        if [ "$_currentRoot" = "apache" ]; then
          wellknown_path="$PROTOCOL_DIR"
        else
          wellknown_path="$_currentRoot/.well-known/protocol-challenge"
          if [ ! -d "$_currentRoot/.well-known" ]; then
            removelevel='1'
          elif [ ! -d "$_currentRoot/.well-known/protocol-challenge" ]; then
            removelevel='2'
          else
            removelevel='3'
          fi
      ...
      protocolvalidationv1="$(printf "%s" "$keyauthorization" | _digest "sha256" "hex")"
      _debug protocolvalidationv1 "$protocolvalidationv1"
      if ! _starttlsserver "$d" "" "$Le_TLSPort" "$keyauthorization" "$_ncaddr" "$protocolvalidationv1"; then
      ...
      if _contains "$status" "invalid"; then
        ...
        if [ "$DEBUG" ]; then
          if [ "$vtype" = "$VTYPE_HTTP" ]; then
            ...
            _get "http://$d/.well-known/protocol-challenge/$token" "" 1
          fi
        fi
        _clearupwebbroot "$_currentRoot" "$removelevel" "$token"
        _clearup
  ...
  if [ "$Le_LinkCert" ]; then
    ...
    if [ ! "$USER_PATH" ] || [ ! "$_PROTOCOL_IN_CRON" ]; then
      USER_PATH="$PATH"
...
renew() {
  ...
  _clearAPI
  _clearCA
  export PROTOCOL_DIRECTORY="$Le_API"
  ...
  if [ -z "$FORCE" ] && [ "$Le_NextRenewTime" ] && [ "$(_time)" -lt "$Le_NextRenewTime" ]; then
    ...
    _info "Add '$(__red '--force')' to force renewal."
    if [ -z "$_PROTOCOL_IN_RENEWALL" ]; then
  ...
  if [ "$_PROTOCOL_IN_CRON" = "1" ] && [ -z "$Le_CertCreateTime" ]; then
    _info "Skipping invalid cert for: $Le_Domain"
    return $RENEW_SKIP
  fi

  _PROTOCOL_IS_RENEW="1"
...
renewAll() {
  ...
  export _PROTOCOL_IN_RENEWALL=1
...
_installcert() {
  ...
  if [ "$_real_cert" ]; then
    ...
    if [ -f "$_real_cert" ] && [ ! "$_PROTOCOL_IS_RENEW" ]; then
  ...
  if [ "$_real_ca" ]; then
    ...
    if [ "$_real_ca" = "$_real_cert" ]; then
      ...
      if [ -f "$_real_ca" ] && [ ! "$_PROTOCOL_IS_RENEW" ]; then
  ...
  if [ "$_real_key" ]; then
    ...
    if [ -f "$_real_key" ] && [ ! "$_PROTOCOL_IS_RENEW" ]; then
  ...
  if [ "$_real_fullchain" ]; then
    ...
    if [ -f "$_real_fullchain" ] && [ ! "$_PROTOCOL_IS_RENEW" ]; then
...
revoke() {
  ...
  if [ "$Le_API" ]; then
    if [ "$Le_API" != "$PROTOCOL_DIRECTORY" ]; then
      _clearAPI
    fi
    export PROTOCOL_DIRECTORY="$Le_API"
    #reload ca configs
...
_deactivate() {
  ...
  if ! _send_signed_request "$PROTOCOL_NEW_ORDER" "{\"identifiers\": [$_identifiers]}"; then
...
_precheck() {
  ...
  if ! _exists "${PROTOCOL_OPENSSL_BIN:-openssl}"; then
    _err "Please install openssl first. PROTOCOL_OPENSSL_BIN=$PROTOCOL_OPENSSL_BIN"
...
install() {
  ...
  if [ "$_PROTOCOL_IN_CRON" != "1" ] && [ -z "$_noprofile" ]; then
...
Parameters:
  ...
  --server <server_uri>             PROTOCOL Directory Resource URI. (default: $DEFAULT_CA)
                                      See: $_SERVER_WIKI
...
_getRepoHash() {
  _hash_path=$1
  shift
  _hash_url="${PROJECT_API:-https://api.github.com/repos/khulnasoft}/$PROJECT_NAME/git/refs/$_hash_path"
...
_processAccountConf() {
  ...
  if [ "$_openssl_bin" ]; then
    _saveaccountconf "PROTOCOL_OPENSSL_BIN" "$_openssl_bin"
  elif [ "$PROTOCOL_OPENSSL_BIN" ] && [ "$PROTOCOL_OPENSSL_BIN" != "$DEFAULT_OPENSSL_BIN" ]; then
    _saveaccountconf "PROTOCOL_OPENSSL_BIN" "$PROTOCOL_OPENSSL_BIN"
  fi
  ...
  if [ "$_use_wget" ]; then
    _saveaccountconf "PROTOCOL_USE_WGET" "$_use_wget"
  elif [ "$PROTOCOL_USE_WGET" ]; then
    _saveaccountconf "PROTOCOL_USE_WGET" "$PROTOCOL_USE_WGET"
  fi
...
_checkSudo() {
  ...
  if [ "$SUDO_GID" ] && [ "$SUDO_COMMAND" ] && [ "$SUDO_USER" ] && [ "$SUDO_UID" ]; then
    ...
    if [ -n "$SUDO_COMMAND" ]; then
      #it's a normal user doing "sudo su", or `sudo -i` or `sudo -s`, or `sudo su protocoluser1`
  ...
  for snames in $CA_NAMES; do
    ...
    for sname in $(echo "$snames" | tr ',' ' '); do
      if [ "$_server_lower" = "$sname" ]; then
        ...
        PROTOCOL_DIRECTORY="$_serverdir"
        export PROTOCOL_DIRECTORY
        return
      fi
    done
  done
  PROTOCOL_DIRECTORY="$_server"
  export PROTOCOL_DIRECTORY
}
...
#set default ca to $PROTOCOL_DIRECTORY
setdefaultca() {
  if [ -z "$PROTOCOL_DIRECTORY" ]; then
    _err "Please provide a --server parameter."
    return 1
  fi
  _saveaccountconf "DEFAULT_PROTOCOL_SERVER" "$PROTOCOL_DIRECTORY"
  _info "Changed default CA to: $(__green "$PROTOCOL_DIRECTORY")"
}
...
_process() {
  ...
  while [ ${#} -gt 0 ]; do
    ...
    --no-profile | --noprofile)
      ...
      ;;
    --no-color)
      export PROTOCOL_NO_COLOR=1
      ;;
    --force-color)
      export PROTOCOL_FORCE_COLOR=1
      ;;
    --ecc)
      _ecc="isEcc"
      ;;
    ...
    --openssl-bin)
      _openssl_bin="$2"
      PROTOCOL_OPENSSL_BIN="$_openssl_bin"
      shift
      ;;
    --use-wget)
      _use_wget="1"
