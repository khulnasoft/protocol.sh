...
Site: ISPConfig.org
Docs: github.com/khulnasoft/protocol.sh/wiki/dnsapi#dns_ispconfig
Options:
...
#Usage: dns_myapi_add   _protocol-challenge.www.domain.com   "XKrxpRBosdIKFzxW_CT3KLZNf6q0HG9i01zxXp5CPBs"
dns_ispconfig_add() {
  fulldomain="${1}"
  txtvalue="${2}"
...
#Usage: dns_myapi_rm   _protocol-challenge.www.domain.com
dns_ispconfig_rm() {
  fulldomain="${1}"
...
_ISPC_getZoneInfo() {
  ...
  while [ "${zoneEnd}" = false ]; do
    # we can strip the first part of the fulldomain, since it's just the _protocol-challenge string
    curZone="${curZone#*.}"
...
_ISPC_addTxt() {
  ...
  case "${record_id}" in
  '' | *[!0-9]*)
    _err "Couldn't add PROTOCOL Challenge TXT record to zone."
    return 1
    ;;
  *) _info "Added PROTOCOL Challenge TXT record to zone." ;;
  esac
}
...
_ISPC_rmTxt() {
  ...
  if _contains "${curResult}" '"code":"ok"'; then
    ...
    *)
      ...
      _debug "Result of _ISPC_rmTxt: '$curResult'"
      if _contains "${curResult}" '"code":"ok"'; then
        _info "Removed PROTOCOL Challenge TXT record from zone."
      else
        _err "Couldn't remove PROTOCOL Challenge TXT record from zone."
        return 1
      fi
      ;;
    esac
