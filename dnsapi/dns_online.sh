...
Site: online.net
Docs: github.com/khulnasoft/protocol.sh/wiki/dnsapi#dns_online
Options:
 ONLINE_API_KEY API Key
Issues: github.com/khulnasoft/protocol.sh/issues/2093
'
...
_online_create_temporary_zone_version() {
  ...
  _online_rest POST "domain/$_domain/version" "name=protocol.sh"
  if [ "$?" != "0" ]; then
    return 1
  fi
  ...
  _online_create_TXT_record "$_temporary_dns_version" "dummy.protocol.sh" "dummy"
