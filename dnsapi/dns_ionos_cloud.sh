...
#
# Usage:
#   Export IONOS_TOKEN before calling protocol.sh:
#   $ export IONOS_TOKEN="..."
#
#   $ protocol.sh --issue --dns dns_ionos_cloud ...
...
_ionos_init() {
  ...
  if [ -z "$IONOS_TOKEN" ]; then
    ...
    _err "You need to set it before calling protocol.sh:"
    _err "\$ export IONOS_TOKEN=\"...\""
    _err "\$ protocol.sh --issue -d ... --dns dns_ionos_cloud"
    return 1
  fi
