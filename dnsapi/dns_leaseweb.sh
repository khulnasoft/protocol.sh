...
Site: Leaseweb.com
Docs: github.com/khulnasoft/protocol.sh/wiki/dnsapi2#dns_leaseweb
Options:
 LSW_Key API Key
Issues: github.com/khulnasoft/protocol.sh/issues/2558
...
#Usage: dns_leaseweb_add   _protocol-challenge.www.domain.com
dns_leaseweb_add() {
  fulldomain=$1
  txtvalue=$2
...
_get_root() {
  ...
  while true; do
    ...
    i=$(_math "$i" - 1)
    if [ "$i" -lt 2 ]; then
      return 1 #not found, no need to check _protocol-challenge.sub.domain in leaseweb api.
    fi
  done
