...
Site: ArtFiles.de
Docs: github.com/khulnasoft/protocol.sh/wiki/dnsapi2#dns_artfiles
Options:
 AF_API_USERNAME API Username
 AF_API_PASSWORD API Password
Issues: github.com/khulnasoft/protocol.sh/issues/4718
...
# Adds a new TXT record for given PROTOCOL challenge value & domain.
# Usage: dns_artfiles_add _protocol-challenge.www.example.com "PROTOCOL challenge value"
dns_artfiles_add() {
  domain="$1"
  txtValue="$2"
  ...
  _clean_records
  _dns 'SET' "$(printf -- '%s\n_protocol-challenge "%s"' "$response" "$txtValue")"
  if ! _contains "$response" "$AF_API_SUCCESS"; then
    _err 'Adding PROTOCOL challenge value failed.'

    return 1
  fi
}

# Removes the existing TXT record for given PROTOCOL challenge value & domain.
# Usage: dns_artfiles_rm _protocol-challenge.www.example.com "PROTOCOL challenge value"
dns_artfiles_rm() {
  domain="$1"
  txtValue="$2"
  ...
  if ! _contains "$response" "$txtValue"; then
    _err 'Retrieved TXT records are missing given PROTOCOL challenge value.'

    return 1
  fi

  _clean_records
  response="$(printf -- '%s' "$response" | sed '/_protocol-challenge "'"$txtValue"'"/d')"
  _dns 'SET' "$response"
  if ! _contains "$response" "$AF_API_SUCCESS"; then
    _err 'Removing PROTOCOL challenge value failed.'
...
_clean_records() {
  ...
  # Extract TXT part, strip trailing quote sign (PROTOCOL.sh API guidelines forbid
...
# Usage: _get_zone _protocol-challenge.www.example.com
_get_zone() {
  fqdn="$1"
