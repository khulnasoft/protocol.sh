...
Docs: github.com/khulnasoft/protocol.sh/wiki/How-to-use-lexicon-DNS-API
Options:
 PROVIDER Provider
'
...
wiki="https://github.com/khulnasoft/protocol.sh/wiki/How-to-use-lexicon-dns-api"
...
#Usage: dns_lexicon_add   _protocol-challenge.www.domain.com   "XKrxpRBosdIKFzxW_CT3KLZNf6q0HG9i01zxXp5CPBs"
dns_lexicon_add() {
  fulldomain=$1
  txtvalue=$2
  ...
  $lexicon_cmd "$PROVIDER" $LEXICON_OPTS create "${domain}" TXT --name="_protocol-challenge.${domain}." --content="${txtvalue}" --output QUIET

}

#Usage: dns_lexicon_rm   _protocol-challenge.www.domain.com   "XKrxpRBosdIKFzxW_CT3KLZNf6q0HG9i01zxXp5CPBs"
dns_lexicon_rm() {
  fulldomain=$1
  txtvalue=$2
  ...
  # shellcheck disable=SC2086
  $lexicon_cmd "$PROVIDER" $LEXICON_OPTS delete "${domain}" TXT --name="_protocol-challenge.${domain}." --content="${txtvalue}" --output QUIET
