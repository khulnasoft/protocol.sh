...
Docs: github.com/khulnasoft/protocol.sh/wiki/dnsapi#dns_azure
Options:
...
wiki=https://github.com/khulnasoft/protocol.sh/wiki/How-to-use-Azure-DNS

########  Public functions #####################

# Usage: add  _protocol-challenge.www.domain.com   "XKrxpRBosdIKFzxW_CT3KLZNf6q0HG9i01zxXp5CPBs"
...
dns_azure_add() {
  ...
  protocolRecordURI="https://management.azure.com$(printf '%s' "$_domain_id" | sed 's/\\//g')/TXT/$_sub_domain?api-version=2017-09-01"
  _debug "$protocolRecordURI"
  # Get existing TXT record
  _azure_rest GET "$protocolRecordURI" "" "$accesstoken"
  ...
  if [ "$_code" = "200" ]; then
    ...
    _debug "$vlist"
    existingts="$(echo "$response" | _egrep_o "\"protocoltscheck\"\\s*:\\s*\"[^\"]*\"" | _head_n 1 | cut -d : -f 2 | tr -d "\"")"
    if [ -z "$existingts" ]; then
      # the record was not created by protocol.sh. Copy the exisiting entires
  ...
  body="{\"properties\":{\"metadata\":{\"protocoltscheck\":\"$timestamp\"},\"TTL\":10, \"TXTRecords\":[$values]}}"
  _azure_rest PUT "$protocolRecordURI" "$body" "$accesstoken"
...
dns_azure_rm() {
  ...
  if [ "$_code" = "200" ]; then
    ...
    if [ -z "$values" ]; then
      ...
      _debug "removing validation record completely $protocolRecordURI"
      _azure_rest DELETE "$protocolRecordURI" "" "$accesstoken"
