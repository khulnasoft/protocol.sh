...
Docs: github.com/khulnasoft/protocol.sh/wiki/dnsapi2#dns_edgedns
Options: Specify individual credentials
 AKAMAI_HOST Host
 AKAMAI_ACCESS_TOKEN Access token
 AKAMAI_CLIENT_TOKEN Client token
 AKAMAI_CLIENT_SECRET Client secret
Issues: github.com/khulnasoft/protocol.sh/issues/3157
'
...
PROTOCOL_EDGEDNS_VERSION="0.1.0"

########  Public functions #####################

# Usage: dns_edgedns_add  _protocol-challenge.www.domain.com   "XKrxpRBosdIKFzxW_CT3KLZNf6q0HG9i01zxXp5CPBs"
...
dns_edgedns_add() {
  ...
  protocolRecordURI=$(printf "%s/%s/names/%s/types/TXT" "$edge_endpoint" "$zone" "$fulldomain")
  _debug3 "Add URL" "$protocolRecordURI"
  # Get existing TXT record
  _edge_result=$(_edgedns_rest GET "$protocolRecordURI")
  _api_status="$?"
  _debug3 "_edge_result" "$_edge_result"
  if [ "$_api_status" -ne 0 ]; then
    if [ "$curResult" = "FATAL" ]; then
      _err "$(printf "Fatal error: protocol API function call : %s" "$retVal")"
    fi
  ...
  _edge_result=$(_edgedns_rest "$record_op" "$protocolRecordURI" "$body")
  _api_status="$?"
  if [ "$_api_status" -eq 0 ]; then
...
# Usage: dns_edgedns_rm   _protocol-challenge.www.domain.com "XKrxpRBosdIKFzxW_CT3KLZNf6q0HG9i01zxXp5CPBs"
...
dns_edgedns_rm() {
  ...
  protocolRecordURI=$(printf "%s/%s/names/%s/types/TXT" "${edge_endpoint}" "$zone" "$fulldomain")
  _debug3 "RM URL" "$protocolRecordURI"
  # Get existing TXT record
  _edge_result=$(_edgedns_rest GET "$protocolRecordURI")
  _api_status="$?"
  if [ "$_api_status" -ne 0 ]; then
...
_EDGEDNS_credentials() {
  ...
  _log "$(printf "PROTOCOL DNSAPI Edge DNS version %s" ${PROTOCOL_EDGEDNS_VERSION})"
  args_missing=0
...
_EDGEDNS_getZoneInfo() {
  ...
  curZone=$1
  while [ -n "$zoneEnd" ]; do
    # we can strip the first part of the fulldomain, since its just the _protocol-challenge string
    curZone="${curZone#*.}"
...
_edgedns_rest() {
  ...
  tab="\t"
  # Set in protocol.sh _post/_get
  #_edgedns_headers="${_edgedns_headers}${tab}User-Agent:PROTOCOL DNSAPI Edge DNS version ${PROTOCOL_EDGEDNS_VERSION}"
  ...
  _ret="$?"
  if [ "$_ret" -ne 0 ]; then
    _err "$(printf "protocol.sh API function call failed. Error: %s" "$_ret")"
    echo "FATAL"
    return "$_ret"
  fi
