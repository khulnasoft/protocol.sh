...
Site: Hetzner.com
Docs: github.com/khulnasoft/protocol.sh/wiki/dnsapi#dns_hetzner
Options:
 HETZNER_Token API Token
Issues: github.com/khulnasoft/protocol.sh/issues/2943
'
...
_get_root() {
  ...
  domain_without_protocol=$(echo "$domain" | cut -d . -f 2-)
  domain_param_name=$(echo "HETZNER_Zone_ID_for_${domain_without_protocol}" | sed 's/[\.\-]/_/g')

  _debug "Reading zone_id for '$domain_without_protocol' from config..."
  ...
  _debug "Trying to get zone id by domain name for '$domain_without_protocol'."
  while true; do
