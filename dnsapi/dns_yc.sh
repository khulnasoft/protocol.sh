...
Docs: github.com/khulnasoft/protocol.sh/wiki/dnsapi2#dns_yc
Options:
...
Issues: github.com/khulnasoft/protocol.sh/issues/4210
'
...
_yc_login() {
  ...
  #signature=$(printf "%s.%s" "$header" "$payload" | ${PROTOCOL_OPENSSL_BIN:-openssl} dgst -sign "$YC_SA_Key_File -sha256 -sigopt rsa_padding_mode:pss -sigopt rsa_pss_saltlen:-1" | _base64 | _url_replace )
