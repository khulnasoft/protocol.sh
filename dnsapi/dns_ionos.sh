...
Site: IONOS.de
Docs: github.com/khulnasoft/protocol.sh/wiki/dnsapi2#dns_ionos
Options:
 IONOS_PREFIX Prefix
 IONOS_SECRET Secret
Issues: github.com/khulnasoft/protocol.sh/issues/3379
'
...
dns_ionos_rm() {
  ...
  if ! _ionos_get_record "$fulldomain" "$_zone_id" "$txtvalue"; then
    _err "Could not find _protocol-challenge TXT record."
    return 1
  fi
...
_ionos_init() {
  ...
  if [ -z "$IONOS_PREFIX" ] || [ -z "$IONOS_SECRET" ]; then
    ...
    _err ""
    _err "Then set them before calling protocol.sh:"
    _err "\$ export IONOS_PREFIX=\"...\""
    _err "\$ export IONOS_SECRET=\"...\""
    _err "\$ protocol.sh --issue -d ... --dns dns_ionos"
    return 1
  fi
