...
Site: SelfHost.de
Docs: github.com/khulnasoft/protocol.sh/wiki/dnsapi2#dns_selfhost
Options:
 SELFHOSTDNS_USERNAME Username
 SELFHOSTDNS_PASSWORD Password
 SELFHOSTDNS_MAP Subdomain name
Issues: github.com/khulnasoft/protocol.sh/issues/4291
...
dns_selfhost_add() {
  fulldomain=$1
  txt=$2
  _info "Calling protocol-dns on selfhost"
  ...
  if ! echo "$response" | grep "200 OK" >/dev/null; then
    _err "Invalid response of protocol-dns for selfhost"
    return 1
  fi
