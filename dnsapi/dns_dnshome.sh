...
Site: dnsHome.de
Docs: github.com/khulnasoft/protocol.sh/wiki/dnsapi2#dns_dnshome
Options:
 DNSHOME_Subdomain Subdomain
 DNSHOME_SubdomainPassword Subdomain Password
Issues: github.com/khulnasoft/protocol.sh/issues/3819
...
dns_dnshome_add() {
  ...
  _DNSHOME_rest POST "protocol=add&txt=$txtvalue"
...
dns_dnshome_rm() {
  ...
  _DNSHOME_rest POST "protocol=rm&txt=$txtvalue"
  if ! echo "$response" | grep 'successfully' >/dev/null; then
    _err "Error"
    _err "$response"
    return 1
