...
# protocol.sh DNS API for Timeweb Cloud provider (https://timeweb.cloud).
#
...
# 2.  As a "TW_Token" config entry in protocol.sh account config file
#     (usually located at ~/.protocol.sh/account.conf by default).

TW_Api="https://api.timeweb.cloud/api/v1"

################  Public functions ################

# Adds an PROTOCOL DNS-01 challenge DNS TXT record via the Timeweb Cloud API.
#
# Param1: The PROTOCOL DNS-01 challenge FQDN.
# Param2: The value of the PROTOCOL DNS-01 challenge TXT record.
#
# Example: dns_timeweb_add "_protocol-challenge.sub.domain.com" "D-52Wm...4uYM"
dns_timeweb_add() {
  _debug "$(__green "Timeweb DNS API"): \"dns_timeweb_add\" started."

  _timeweb_set_protocol_fqdn "$1" || return 1
  _timeweb_set_protocol_txt "$2" || return 1
  _timeweb_check_token || return 1
  _timeweb_split_protocol_fqdn || return 1
...
# Param2: The value of the PROTOCOL DNS-01 challenge TXT record.
#
# Example: dns_timeweb_rm "_protocol-challenge.sub.domain.com" "D-52Wm...4uYM"
dns_timeweb_rm() {
...
# Checks and sets the PROTOCOL DNS-01 challenge FQDN.
#
# Param1: The PROTOCOL DNS-01 challenge FQDN.
#
# Example: _timeweb_set_protocol_fqdn "_protocol-challenge.sub.domain.com"
#
# Sets the "Protocol_Fqdn" variable (_protocol-challenge.sub.domain.com)
_timeweb_set_protocol_fqdn() {
  Protocol_Fqdn=$1
  _debug "Setting PROTOCOL DNS-01 challenge FQDN \"$Protocol_Fqdn\"."
  [ -z "$Protocol_Fqdn" ] && {
    _err "PROTOCOL DNS-01 challenge FQDN is empty."
    return 1
  }
  return 0
}

# Checks and sets the value of the PROTOCOL DNS-01 challenge TXT record.
#
# Param1: Value of the PROTOCOL DNS-01 challenge TXT record.
#
# Example: _timeweb_set_protocol_txt "D-52Wm...4uYM"
#
# Sets the "Protocol_Txt" variable to the provided value (D-52Wm...4uYM)
_timeweb_set_protocol_txt() {
  Protocol_Txt=$1
  _debug "Setting the value of the PROTOCOL DNS-01 challenge TXT record to \"$Protocol_Txt\"."
  [ -z "$Protocol_Txt" ] && {
    _err "PROTOCOL DNS-01 challenge TXT record value is empty."
    return 1
  }
  return 0
}
...
# Adds or updates the token in the protocol.sh account configuration.
...
# Divides the PROTOCOL DNS-01 challenge FQDN into its main domain and subdomain components.
_timeweb_split_protocol_fqdn() {
  _debug "Trying to divide \"$Protocol_Fqdn\" into its main domain and subdomain components."
  ...
  _err "Failed to divide \"$Protocol_Fqdn\" into its main domain and subdomain components."
  return 1
}
...
_timeweb_get_dns_txt() {
  _debug "Trying to locate a DNS TXT record with the value \"$Protocol_Txt\"."
...
# Verifies whether the domain is the primary domain for the PROTOCOL DNS-01 challenge FQDN.
# The requirement is that the provided domain is the top-level domain
# for the PROTOCOL DNS-01 challenge FQDN.
#
# Param 1: Domain object returned by Timeweb Cloud API.
#
# Sets the "TW_Main_Domain" variable (e.g. "_protocol-challenge.s1.domain.co.uk" → "domain.co.uk").
# Sets the "TW_Subdomains" variable (e.g. "_protocol-challenge.s1.domain.co.uk" → "_protocol-challenge.s1").
_timeweb_is_main_domain() {
  _debug "Checking if \"$1\" is the main domain of the PROTOCOL DNS-01 challenge FQDN."
  ...
  ! echo ".$Protocol_Fqdn" | grep -qi "\.$1$" && {
    _debug "Domain does not match the PROTOCOL DNS-01 challenge FQDN. Skipping domain."
    return 1
  }
  ...
  _debug "Matched domain. PROTOCOL DNS-01 challenge FQDN  split as [$TW_Subdomains].[$TW_Main_Domain]."
  return 0
}

# Verifies whether a DNS record was previously added based on the following criteria:
# - The value matches the PROTOCOL DNS-01 challenge TXT record value;
# - The record type is TXT;
# - The subdomain matches the PROTOCOL DNS-01 challenge FQDN.
#
