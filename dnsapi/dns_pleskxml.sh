...
Site: Plesk.com
Docs: github.com/khulnasoft/protocol.sh/wiki/dnsapi2#dns_pleskxml
Options:
...
Issues: github.com/khulnasoft/protocol.sh/issues/2577
...
##  For example, to add a TXT record to DNS alias domain "protocol-alias.com" would be a valid Plesk action.
##  So this API module can handle such a request, if needed.

##  For PROTOCOL v2 purposes, new TXT records are appended when added, and removing one TXT record will not affect any other TXT records.
...
# Get a list of domains that PLESK can manage, so we can check root domain + host for protocol.sh
...
# Get a list of additional domains that PLESK can manage, so we can check root domain + host for protocol.sh
# No params.
...
# PARAMS = (1) Plesk internal domain ID, (2) "hostname" for the new record, eg '_protocol_challenge', (3) TXT record value
...
#Usage: dns_pleskxml_add   _protocol-challenge.www.domain.com   "XKrxpRBosdIKFzxW_CT3KLZNf6q0HG9i01zxXp5CPBs"
dns_pleskxml_add() {
  fulldomain=$1
  txtvalue=$2
...
#Usage: dns_pleskxml_rm   _protocol-challenge.www.domain.com   "XKrxpRBosdIKFzxW_CT3KLZNf6q0HG9i01zxXp5CPBs"
dns_pleskxml_rm() {
  fulldomain=$1
  txtvalue=$2
...
_credential_check() {
  ...
  if [ -z "$pleskxml_user" ] || [ -z "$pleskxml_pass" ] || [ -z "$pleskxml_uri" ]; then
    ...
    _err "Instructions are in the 'dns_pleskxml' plugin source code or in the protocol.sh documentation."
    return 1
  fi
  ...
  _debug "Test login to Plesk XML API successful. Login credentials and URI successfully saved to the protocol.sh configuration file for future use."
...
_pleskxml_get_root_domain() {
  ...
  if [ "$(_countdots "$original_full_domain_name")" -eq 0 ]; then
    _err "Invalid domain. The PROTOCOL domain must contain at least two parts (aa.bb) to identify a domain and tld for the TXT record."
    return 1
  fi
