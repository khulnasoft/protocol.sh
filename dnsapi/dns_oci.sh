...
Docs: github.com/khulnasoft/protocol.sh/wiki/How-to-use-Oracle-Cloud-Infrastructure-DNS
Options:
...
Issues: github.com/khulnasoft/protocol.sh/issues/3540
...
_fingerprint() {
  ...
  printf "%s" "$pkey" | ${PROTOCOL_OPENSSL_BIN:-openssl} rsa -pubout -outform DER 2>/dev/null | ${PROTOCOL_OPENSSL_BIN:-openssl} md5 -c | cut -d = -f 2 | tr -d ' '
