...
Docs: github.com/khulnasoft/protocol.sh/wiki/dnsapi#dns_openprovider
Options:
 OPENPROVIDER_USER Username
 OPENPROVIDER_PASSWORDHASH Password hash
Issues: github.com/khulnasoft/protocol.sh/issues/2104
...
#Usage: dns_openprovider_add   _protocol-challenge.www.domain.com   "XKrxpRBosdIKFzxW_CT3KLZNf6q0HG9i01zxXp5CPBs"
dns_openprovider_add() {
  ...
  _debug "Creating protocol record"
  protocol_record="$(echo "$fulldomain" | sed -e "s/.$_domain_name.$_domain_extension$//")"
  _openprovider_request "$(printf '<modifyZoneDnsRequest><domain><name>%s</name><extension>%s</extension></domain><type>master</type><records><array>%s<item><name>%s</name><type>TXT</type><value>%s</value><ttl>600</ttl></item></array></records></modifyZoneDnsRequest>' "$_domain_name" "$_domain_extension" "$existing_items" "$protocol_record" "$txtvalue")"
...
dns_openprovider_rm() {
  ...
  while true; do
    ...
    # Remove protocol records from items
    items="$response"
    while true; do
      item="$(echo "$items" | _egrep_o '<openXML>.*<\/openXML>' | sed -n 's/.*\(<item>.*<\/item>\).*/\1/p')"
      _debug existing_items "$existing_items"
      ...
      if ! echo "$item" | grep -v "$fulldomain"; then
        _debug "protocol record, skipping" "$item"
        continue
      fi
  ...
  _debug "Removing protocol record"
