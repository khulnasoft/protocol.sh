...
Site: DNS.Services
Docs: github.com/khulnasoft/protocol.sh/wiki/dnsapi2#dns_dnsservices
Options:
 DnsServices_Username Username
 DnsServices_Password Password
Issues: github.com/khulnasoft/protocol.sh/issues/4152
...
#Usage: dns_dnsservices_add  _protocol-challenge.www.domain.com   "XKrxpRBosdIKFzxW_CT3KLZNf6q0HG9i01zxXp5CPBs"
dns_dnsservices_add() {
  fulldomain="$1"
  txtvalue="$2"

  _info "Using dns.services to create PROTOCOL DNS challenge"
  _debug2 add_fulldomain "$fulldomain"
  _debug2 add_txtvalue "$txtvalue"

  # Read username/password from environment or .protocol.sh/accounts.conf
...
_get_root() {
  ...
  subDomainNameClean="$(printf "%s\n" "$domain" | sed "s,_protocol-challenge.,,g")"
