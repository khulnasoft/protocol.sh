...
# test that you can login to USER@SERVER from the host running protocol.sh before
# using this script.
#
...
# export DEPLOY_SSH_BACKUP_PATH=".protocol_ssh_deploy"  # path on remote system. Defaults to .protocol_ssh_deploy
...
ssh_deploy() {
  ...
  # BACKUP_PATH is optional. If not provided then default to previously saved value or .protocol_ssh_deploy
  ...
  if [ -z "$DEPLOY_SSH_BACKUP_PATH" ]; then
    DEPLOY_SSH_BACKUP_PATH=".protocol_ssh_deploy"
  fi
