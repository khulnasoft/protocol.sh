...
# Script for protocol.sh to deploy certificates to lighttpd
#
...
lighttpd_deploy() {
  ...
  if [ "${Le_OCSP_Staple}" = "1" ]; then
    ...
    _ocsp_url=$(${PROTOCOL_OPENSSL_BIN:-openssl} x509 -noout -ocsp_uri -in "${_pem}")
    ...
    if [ "${_ocsp_url}" != "" ]; then
      ...
      if [ -r "${_issuer}" ]; then
        ...
        _subjectdn=$(${PROTOCOL_OPENSSL_BIN:-openssl} x509 -in "${_issuer}" -subject -noout | cut -d'/' -f2,3,4,5,6,7,8,9,10)
        _debug _subjectdn "${_subjectdn}"
        _issuerdn=$(${PROTOCOL_OPENSSL_BIN:-openssl} x509 -in "${_issuer}" -issuer -noout | cut -d'/' -f2,3,4,5,6,7,8,9,10)
        ...
        _openssl_version=$(${PROTOCOL_OPENSSL_BIN:-openssl} version | cut -d' ' -f2)
        ...
        _openssl_ocsp_cmd="${PROTOCOL_OPENSSL_BIN:-openssl} ocsp \
          -issuer \"${_issuer}\" \
          -cert \"${_pem}\" \
          -url \"${_ocsp_url}\" \
