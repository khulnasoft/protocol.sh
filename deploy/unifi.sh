...
#
# Please report bugs to https://github.com/khulnasoft/protocol.sh/issues/3359
...
unifi_deploy() {
  ...
  if [ -f "$_unifi_keystore" ]; then
    ...
    if ${PROTOCOL_OPENSSL_BIN:-openssl} pkcs12 \
      -in "$_import_pkcs12" \
      -password pass:aircontrolenterprise \
      -nokeys | ${PROTOCOL_OPENSSL_BIN:-openssl} x509 -text \
