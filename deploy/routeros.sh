...
#
# ```sh
# protocol.sh --deploy -d ftp.example.com --deploy-hook routeros
# ```
#
...
# Example:
# ```ssh
# export ROUTER_OS_SSH_CMD="ssh -i /protocol.sh/.ssh/router.example.com -o UserKnownHostsFile=/protocol.sh/.ssh/known_hosts"
# export ROUTER_OS_SCP_CMD="scp -i /protocol.sh/.ssh/router.example.com -o UserKnownHostsFile=/protocol.sh/.ssh/known_hosts"
# ````
#
...
comment=\"generated by routeros deploy script in protocol.sh\" \
