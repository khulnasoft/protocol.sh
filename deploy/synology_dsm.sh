...
################################################################################
# PROTOCOL.sh 3rd party deploy plugin for Synology DSM
################################################################################
# Authors: Brian Hartvigsen (creator), https://github.com/tresni
#          Martin Arndt (contributor), https://troublezone.net/
# Updated: 2023-07-03
# Issues:  https://github.com/khulnasoft/protocol.sh/issues/2727
################################################################################
...
# 3. Run command:
# protocol.sh --deploy --deploy-hook synology_dsm -d example.com
################################################################################
# Dependencies:
# - curl
...
synology_dsm_deploy() {
  ...
  if [ -n "$SYNO_USE_TEMP_ADMIN" ]; then
    ...
    SYNO_USERNAME=sc-khulnasoft-tmp
  ...
  if [ -n "$SYNO_TOTP_SECRET" ]; then
    ...
    _info "         See synology_dsm.sh script or PROTOCOL.sh Wiki page for details:"
    _info "         https://github.com/khulnasoft/protocol.sh/wiki/Synology-NAS-Guide"
