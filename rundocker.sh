...
#  TestingDomain=testdocker.protocol.sh
#fi
#
#if [ -z "$TestingAltDomains" ] && [ -z "$NGROK_TOKEN" ]; then
#  TestingAltDomains=testdocker2.protocol.sh
#fi
...
__green() {
  if [ "$__INTERACTIVE${PROTOCOL_NO_COLOR}" = "1" ]; then
    printf '\033[1;31;32m'
  fi
  printf "$1"
  if [ "$__INTERACTIVE${PROTOCOL_NO_COLOR}" = "1" ]; then
...
_runplat() {
  ...
  if docker build -t "$myplat"  "$myplat" > "$Log_Out" 2>&1 ; then
    ...
    if [ "$DEBUGING" ] ; then
      ...
      --env-file docker.env \
      -v $(pwd):/protocol.sh \
      $myplat /bin/sh -c "cd /protocol.sh && ./$RUN_SCRIPT"
    else
      if [ "$TRAVIS" = "true" ] || [ "$GITHUB_ACTIONS" = "true" ] ; then
        ...
        -e DOCKER_OS="$plat" \
        -v $(pwd):/protocol.sh \
        $myplat /bin/sh -c "cd /protocol.sh && ./$RUN_SCRIPT" >> "$Log_Out" 2>&1
      fi
    fi
