...
lezip="https://github.com/khulnasoft/protocol.sh/archive"
legit="https://github.com/khulnasoft/protocol.sh.git"

PROJECT="https://github.com/khulnasoft/protocol.sh"

NGROK_WIKI="https://github.com/khulnasoft/protocol.sh"


DEFAULT_HOME="$HOME/.protocol.sh"
...
if [ -z "$TEST_PROTOCOL_Server" ]; then
  export TEST_PROTOCOL_Server="letsencrypt_test"
fi
...
STAGE_CA="https://protocol-staging.api.letsencrypt.org"
...
_setup() {
  ...
  if command -v tar > /dev/null ; then
    ...
    mv "protocol.sh-$BRANCH" protocol.sh
  elif command -v git > /dev/null ; then
    rm -rf protocol.sh
    git clone $legit -b $BRANCH
  else
    _err "Can not get protocol.sh source code. tar or git must be installed"
    return 1
  fi
  lehome="$DEFAULT_HOME"
  if [ -f "$lehome/$PROJECT_ENTRY" ] ; then
...
le_test_standandalone_renew_v2() {
  ...
  _assertcmd "$lehome/$PROJECT_ENTRY  --server \"$TEST_PROTOCOL_Server\"  --issue -d $TestingDomain --standalone --certpath '$cert' --keypath '$key'  --capath '$ca'  --reloadcmd 'echo this is reload'  --fullchainpath  '$full'" ||  return
  ...
  sleep 5
  _assertcmd "$lehome/$PROJECT_ENTRY --renew --server \"$TEST_PROTOCOL_Server\" -d $TestingDomain --force" ||  return
...
le_test_standandalone_renew_localaddress_v2() {
  ...
  ca="$certdir/ca.cer"
  full="$certdir/full.cer"
  _assertcmd "$lehome/$PROJECT_ENTRY  --server \"$TEST_PROTOCOL_Server\"  --issue -d $TestingDomain --standalone --local-address 0.0.0.0 --certpath '$cert' --keypath '$key'  --capath '$ca'  --reloadcmd 'echo this is reload'  --fullchainpath  '$full'" ||  return
...
le_test_standandalone_listen_v4_v2() {
  ...
  ca="$certdir/ca.cer"
  full="$certdir/full.cer"
  _assertcmd "$lehome/$PROJECT_ENTRY  --server \"$TEST_PROTOCOL_Server\"  --issue -d $TestingDomain --standalone --listen-v4 --cert-file '$cert' --key-file '$key'  --ca-file '$ca'  --reloadcmd 'echo this is reload'  --fullchain-file  '$full'" ||  return
...
le_test_standandalone_listen_v6_v2() {
  ...
  ca="$certdir/ca.cer"
  full="$certdir/full.cer"
  _assertcmd "$lehome/$PROJECT_ENTRY  --server \"$TEST_PROTOCOL_Server\"  --issue -d $TestingDomain --standalone --listen-v6 --certpath '$cert' --keypath '$key'  --capath '$ca'  --reloadcmd 'echo this is reload'  --fullchainpath  '$full'" ||  return
...
le_test_standandalone_SAN() {
  ...
  _assertcmd "$lehome/$PROJECT_ENTRY  --server \"$TEST_PROTOCOL_Server\"  --issue -d \"$TestingDomain\" -d \"$TestingAltDomains\" --standalone" ||  return
...
le_test_standandalone_ECDSA_256_renew() {
  ...
  _assertcmd "$lehome/$PROJECT_ENTRY  --server \"$TEST_PROTOCOL_Server\"  --issue -d $TestingDomain --standalone -k ec-256" ||  return
  ...
  sleep 5
  _assertcmd "$lehome/$PROJECT_ENTRY --renew --server \"$TEST_PROTOCOL_Server\" -d $TestingDomain --force --ecc" ||  return
...
le_test_standandalone_ECDSA_256_SAN_renew_v2() {
  ...
  _assertcmd "$lehome/$PROJECT_ENTRY --server \"$TEST_PROTOCOL_Server\"  --issue -d \"$TestingDomain\" -d \"$TestingAltDomains\" --standalone --keylength ec-256 --certpath '$cert' --keypath '$key'  --capath '$ca'  --reloadcmd 'echo this is reload'  --fullchainpath  '$full'" ||  return
  ...
  _assertcmd "$lehome/$PROJECT_ENTRY --renew --server \"$TEST_PROTOCOL_Server\" --ecc -d \"$TestingDomain\" --force" ||  return
...
le_test_standandalone_ECDSA_384() {
  ...
  _assertcmd "$lehome/$PROJECT_ENTRY --server \"$TEST_PROTOCOL_Server\"  --issue --standalone -d \"$TestingDomain\" -k ec-384" ||  return
  _assertcert "$lehome/$TestingDomain$ECC_SUFFIX/$TestingDomain.cer" "$TestingDomain" "$CA_ECDSA" || return
  _assertcert "$lehome/$TestingDomain$ECC_SUFFIX/ca.cer" "$CA_ECDSA" || return
...
le_test_standandalone_renew_idn_v2() {
  ...
  ca="$certdir/ca.cer"
  full="$certdir/full.cer"
  _assertcmd "$lehome/$PROJECT_ENTRY --server \"$TEST_PROTOCOL_Server\"  --issue -d $_test_idn --standalone --certpath '$cert' --keypath '$key'  --capath '$ca'  --reloadcmd 'echo this is reload'  --fullchainpath  '$full'" ||  return
  ...
  sleep 5
  _assertcmd "$lehome/$PROJECT_ENTRY --renew --server \"$TEST_PROTOCOL_Server\" -d $_test_idn --force" ||  return
...
le_test_dnsapi() {
  ...
  if [ "$TEST_DNS_NO_WILDCARD" != "1" ]; then
    ...
    _assertcmd "$lehome/$PROJECT_ENTRY --server \"$TEST_PROTOCOL_Server\"  --issue -d \"$TestingDomain\" -d \"*.$TestingDomain\" --dns $api --dnssleep \"$d_sleep\" " ||  return
    ...
    if [ -z "$TEST_DNS_NO_SUBDOMAIN" ]; then
      _assertcmd "$lehome/$PROJECT_ENTRY --server \"$TEST_PROTOCOL_Server\"  --issue -d \"$TestingDomain\" -d \"www.$TestingDomain\" --dns $api --dnssleep \"$d_sleep\" " ||  return
    else
      _assertcmd "$lehome/$PROJECT_ENTRY --server \"$TEST_PROTOCOL_Server\"  --issue -d \"$TestingDomain\" --dns $api --dnssleep \"$d_sleep\" " ||  return
    fi
    #_assertcert "$lehome/$TestingDomain/$TestingDomain.cer" "$TestingDomain" "$CA" || return
    #_assertcert "$lehome/$TestingDomain/ca.cer" "$CA" || return
    _assertcmd "$lehome/$PROJECT_ENTRY --deactivate -d \"$TestingDomain\" >/dev/null 2>&1"
  ...
  #add a txt record to a subdomain not "_protocol-challenge"
  (
    ...
    if [ "$TokenName5" ]; then
     ...
     record_content="protocolTestTxtRecord_$random_string"
     _assertcmd "$addcommand protocol.shXyzRandomName.$TestingDomain $record_content"  ||  return
     _assertcmd "$rmcommand  protocol.shXyzRandomName.$TestingDomain $record_content"  ||  return
  ) || return
...
le_test_standandalone_ipcert() {
  ...
  _assertcmd "$lehome/$PROJECT_ENTRY --server \"$TEST_PROTOCOL_Server\"  --issue -d $TestingDomain --standalone" ||  return
  #_assertcert "$lehome/$TestingDomain/$TestingDomain.cer" "$TestingDomain" "$CA" || return
  #_assertcert "$lehome/$TestingDomain/ca.cer" "$CA" || return
...
le_test_alpn_ipcert() {
  ...
  _assertcmd "$lehome/$PROJECT_ENTRY --server \"$TEST_PROTOCOL_Server\"  --issue -d $TestingDomain --alpn" ||  return
  #_assertcert "$lehome/$TestingDomain/$TestingDomain.cer" "$TestingDomain" "$CA" || return
  #_assertcert "$lehome/$TestingDomain/ca.cer" "$CA" || return
...
le_test_preferred_chain() {
  ...
  _assertcmd "$lehome/$PROJECT_ENTRY  --server \"$TEST_PROTOCOL_Server\"  --issue -d \"$TestingDomain\" -d \"$TestingAltDomains\" --standalone --preferred-chain \"$TEST_PREFERRED_CHAIN\" " ||  return
  #_assertcert "$lehome/$TestingDomain/$TestingDomain.cer" "$TestingDomain" "$CA" || return
...
le_test_standandalone_ECDSA_256_pkcs12() {
  ...
  _assertcmd "$lehome/$PROJECT_ENTRY  --server \"$TEST_PROTOCOL_Server\"  --to-pkcs12 -d $TestingDomain --password test  --ecc" ||  return
